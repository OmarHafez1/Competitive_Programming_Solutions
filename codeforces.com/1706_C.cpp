//============================================================================
// Author      : Omar_Hafez
// Created     : 22 July 2022 (Friday)  2:34:44 PM
//============================================================================
...
int calc(int i) {
	int dif = -1;
	int ans = 0;
	if(a[i] <= a[i-1]) dif = a[i-1]-a[i];
	if(a[i] <= a[i+1]) dif = max(dif, a[i+1]-a[i]);
	if(dif >= 0) {
		ans += dif+1;
	}
	return ans;
...
int solve(int ind, bool done) {
	if(ind == n-2) return calc(ind);
	if(ind >= n-1) return 0;
	if(dp[ind] != -1) return dp[ind];
	if(done) return dp[ind] = solve(ind+2, 1)+calc(ind);
	return dp[ind] = min(solve(ind+2, 0)+calc(ind), solve(ind+1, 1));
}
