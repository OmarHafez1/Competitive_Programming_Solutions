//============================================================================
// Author      : Omar_Hafez
// Created     : 26 November 2022 (Saturday)  2:15:24 AM
//============================================================================
...
int ok(int left, int right) {
    right--;
    int up = (right*(right+1))/2-((right-n+2)*(right-n+3))/2;
    int down = up-n+2;
    right++;
    int tmp = N-left-right;
    if(down > tmp) return -1;
    else if(up < tmp) return 1;
    return 0;
}
...
void calculate() {
    ...
    while(left < right) {
        mid = (left+right)/2;
        int tmp = ok(mid, mid+n+(n&1));
        if(tmp == 0) {
            cout << mid << " ";
            int b[n-2];
            tmp = 0;
            for(int i = mid+1; i < mid+n-1; i++) {
                b[i-mid-1] = i;
                tmp += i;
            }
            N -= (mid + mid+n+(n&1));
            int w = n-3;
            int dif = mid+n+(n&1)-b[w]-1;
            while(tmp < N) {
                ll lol = min(N-tmp, dif); 
                b[w] += lol;
                tmp += lol;
                w--;
            }
