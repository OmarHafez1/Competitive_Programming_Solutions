//============================================================================
// Author      : Omar_Hafez
// Created     : 30 August 2022 (Tuesday)  5:54:01 PM
//============================================================================
...
struct FenwickTree2D {
    ...
    FenwickTree2D(vector<vector<int>> a) : FenwickTree2D(a.size(), a.back().size()) {
    ...
    int sum(int xl, int yl, int xr, int yr) {
        return sum(xr, yr) - sum(xl - 1, yl - 1);
    }
...
void calculate() {
    int n, q;
    cin >> n >> q;
    FenwickTree2D f1 = FenwickTree2D(1001, 1001);
    vector<pair<int, int>> a(n);
    for(int i = 0; i < n; i++) {
        cin >> a[i].first >> a[i].second;
        f1.add(a[i].first, a[i].second, a[i].first*a[i].second);
    }
    int h1, h2, w1, w2;
    for(int i = 0; i < q; i++) {
        cin >> h1 >> h2 >> w1 >> w2;

        cout << f1.sum(w1-1, w2-1)+f1.sum(h1, h2)-f1.sum(w1-1, h2)-f1.sum(h1, w2-1) << endl;
    }
}
