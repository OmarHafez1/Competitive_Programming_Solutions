//============================================================================
// Author      : Omar_Hafez
// Created     : 07 July 2022 (Thursday)  5:05:53 PM
//============================================================================
...
vector<vll> go, back;
vector<vpll> adj, radj;
ll friends[3];

void dijkstra(ll start, vector<vpll> &pth, vll &save) {
	priority_queue<pair<ll, ll>,vector<pair<ll, ll>>,greater<pair<ll, ll>>> q;
	save[start] = 0;
	q.push({0, start});
	while(!q.empty()) {
		ll des = q.top().second; 
		ll lastCost = q.top().first;
		q.pop();
		if(save[des] < lastCost) continue;
		fe(x, pth[des]) {
			ll newCost = lastCost + x.second;
			if(newCost < save[x.first]) {
				save[x.first] = newCost;
...
int main() { 
    ...
    ll n, m;
    cin >> n >> m;
    go.assign(3, vll(n+1, INF));
    back.assign(3, vll(n+1, INF));
    adj = vector<vpll> (n+1);
    radj = vector<vpll> (n+1);
    ll x, y, c;
    for(int i = 0; i < m; i++) {
    	cin >> x >> y >> c;
    	adj[x].emplace_back(y, c);
    	radj[y].emplace_back(x, c);
    }
    cin >> friends[0] >> friends[1] >> friends[2];
    f0r(i, 3) {
    	dijkstra(friends[i], adj, go[i]), dijkstra(friends[i], radj, back[i]);
    }
    ll ans = INF, ind = -1;
    for(int meetMe = 1; meetMe <= n; meetMe++) {
    	for(int home = 0; home < 3; home++) {
    		...
    		sum += back[home][meetMe] - go[home][meetMe];
    		if(sum < ans) {
    			ans = sum;
    			ind = friends[home];
    		} else if(sum == ans) {
    			ind = min(ind, friends[home]);
    		}
    	}
    }
    cout << ind << " ";
