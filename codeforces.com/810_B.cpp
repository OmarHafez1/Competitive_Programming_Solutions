...
**
**   author:  Omar_Hafez
**   created: 11 February 2022 (Friday)  11:54:29 PM
**
*/
...
int main() { 
  ...
  for(int i = 0; i < n; i++) {
    ...
    b[i].first = min(a[i].first*2, a[i].second) - min(a[i].first, a[i].second);
    b[i].second = i;
  }
  rsor(b);
  ll ans = 0;
  for(int i = 0; i < f; i++) {
    ans += min(a[b[i].second].first*2, a[b[i].second].second);
  }
  for(int i = f; i < n; i++) {
    ans += min(a[b[i].second].first, a[b[i].second].second);
  }
  cout << ans;
}
