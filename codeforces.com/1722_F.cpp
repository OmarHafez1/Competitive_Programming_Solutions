//============================================================================
// Author      : Omar_Hafez
// Created     : 31 August 2022 (Wednesday)  1:19:00 AM
//============================================================================
...
bool clean(vector<vector<bool>> &a, int x, int y) {
    for(int i = 0; i < 4; i++) {
        if(a[x+i][y] || a[x][y+1]) return 0;
    }
    x += 3;
    y += 3;
    for(int i = 0; i < 4; i++) {
        if(x-i >= 0 && a[x-i][y] || y-i >= 0 && a[x][y-i]) return 0;
    }
    return 1;
}
...
void calculate() {
    int n, m;
    cin >> n >> m;
    vector<vector<bool>> a(n+3, vector<bool>(m+3));
    string s;
    for(int i = 1; i <= n; i++) {
        cin >> s;
        for(int j = 1; j <= m; j++) {
    ...
    for(int i = 1; i <= n; i++) {
        for(int j = 1; j <= m; j++) {
            if(a[i][j] && a[i+1][j] && a[i+1][j+1]) {
                ...
                a[i+1][j] = 0;
                a[i+1][j-1] = 0;
            } else if(a[i][j] && a[i+1][j+1] && a[i][j+1]) {
                bool tmp = a[i+2][j-1];
                a[i+2][j-1] = 0;
                if(!clean(a, i-1, j-1)) {
                    cout << "NO";
