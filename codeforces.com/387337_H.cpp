//============================================================================
// Author      : Omar_Hafez
// Created     : 05 July 2022 (Tuesday)  9:44:28 PM
//============================================================================
...
int main() { 
    ...
    mset<int> st;
    while(right < n) {
    	while(left < right && a[right].first-a[left].first > k) {
    	    st.erase(st.find(a[left].second - a[left].first)); 
    		left++;
    	}
    	if(left == right) {
    		st.insert(a[left].second - a[left].first);
    		right++;
    		continue;
    	}
    	while(right < n && a[right].first - a[left].first <= k) {
    		if(st.size() > 0)
    	    	ans = max(ans, a[right].first + a[right].second + *(prev(st.end())));
    		st.insert(a[right].second - a[right].first);
    	    right++;
    	}
    }
    cout << ans;
