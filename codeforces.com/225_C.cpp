...
**
**   author:  Omar_Hafez
**   created: 25 February 2022 (Friday)  6:59:41 PM
**
*/
...
int ans = 0;
vec<vec<vi>> dp;
int solve(int i, bool lst, int len) {
  if(i == m) {
    if(len < x || len > y)
      return inf;
    return 0; 
  }
  if(dp[i][lst][len] != -1) return dp[i][lst][len];
  if(len > y) return inf;
  int a = solve(i+1, lst, len+1);
  if(lst == dot) {
    a += dots[i];
  } else a += n-dots[i];
  int b = INT_MAX;
...
int main() { 
  ...
  cin >> n >> m >> x >> y;
  dp = vec<vec<vi>>(m, vec<vi>(2, vi(m, -1)));
  dots.resize(m);
  string str;
  for(int i = 0; i < n; i++) {
    cin >> str;
  ...
  cout << min(solve(0, dot, 0), solve(0, dash, 0));
}
