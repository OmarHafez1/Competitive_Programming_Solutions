...
**
**   author:  Omar_Hafez
**   created: 25 February 2022 (Friday)  12:05:35 PM
**
*/
...
int calc(int i, int prev) {
  ...
  if(a[i] == 0) return dp[i][prev] = calc(i+1, 0)+1;
  if(a[i] == 1) {
    if(prev == 1) return dp[i][prev] = calc(i+1, 0)+1;
    return dp[i][prev] = calc(i+1, 1); 
  }
  if(a[i] == 2) {
    if(prev == 2) return dp[i][prev] = calc(i+1, 0)+1;
    return dp[i][prev] = calc(i+1, 2);
  }
  if(prev == 1) return dp[i][prev] = min(calc(i+1, 2), calc(i+1, 0)+1);
  if(prev == 0) return dp[i][prev] = min(calc(i+1, 0)+1, min(calc(i+1, 1), calc(i+1, 2)));
  return dp[i][prev] = min(calc(i+1, 1), calc(i+1, 0)+1);
}
