...
**
**   author:  Omar_Hafez
**   created: 12 February 2022 (Saturday)  1:23:35 AM
**
*/
...
int main() { 
  ...
  for(int i = 0; i < t; i++) {
    ...
    cin >> t1 >> t2;
    for(int j = 0; j < k; j++) {
      if(ks[j].first == t1 && ks[j].second == t2) {
        cout << "Waste\n";
        tmp = -1;
        j = k;
        continue;
      }
      if(ks[j].first <= t1 && ks[j].second <= t2) tmp++;
      else if(ks[j].first < t1) tmp++;
    }
    if(tmp == -1) continue;
    tmp = (t1-1)*m+t2-tmp-1;
    cout << (tmp%3 == 0? "Carrots" : tmp%3 == 1? "Kiwis" : "Grapes");
    newl;
  }
