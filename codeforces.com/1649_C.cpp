//============================================================================
// Author      : Omar_Hafez
// Created     : 25 July 2022 (Monday)  10:03:17 AM
//============================================================================
...
signed main() { 
    ...
    int ans = 0;
    for(int i = 0; i < N; i++) {
    	for(int j = 1; j < ai[i].size(); j++) {
    		ans += ai[i][j] * j - ai[i][j-1] * (ai[i].size()-j);
    		ans += aj[i][j] * j - aj[i][j-1] * (aj[i].size()-j);
    	} 
    }
    cout << ans;
}
