...
**
**   author:  Omar_Hafez
**   created: 02 March 2022 (Wednesday)  6:43:02 PM
**
*/
...
int solve(int a) {
  ...
  if(visited[a]) return INF;
  visited[a] = 1;
  if(a == 1) return dp[a] = solve(2)+1;
  if(a == 2) return dp[a] = solve(4)+1;
  return dp[a] = min(solve(a*2), solve(a-1))+1;
}
