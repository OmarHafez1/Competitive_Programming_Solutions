//============================================================================
// Author      : Omar_Hafez
// Created     : 28 April 2023 (Friday)  8:56:42 AM
//============================================================================
...
void calculate() {
    ...
    for(int i = 1; i < w.size()-1; i++) {
        for(int j = 1; j*j <= w[i]; j++) {
            if(w[i]%j != 0) continue;
            if(mp.count(j) && mp.count(w[i]*w[i]/j))
                ans += mp[j]*mp[w[i]]*mp[w[i]*w[i]/j];
            if(mp.count(w[i]/j) && mp.count(w[i]*j))
                if(j*j != w[i] && j > 1)
                    ans += mp[w[i]/j]*mp[w[i]]*mp[w[i]*j];
        }  
    }
    cout << ans;
}
