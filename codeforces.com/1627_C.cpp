//============================================================================
// Author      : Omar_Hafez
// Created     : 04 August 2022 (Thursday)  12:09:24 PM
//============================================================================
...
void calculate() {
    ...
    while(!vis[start]) {
        if(vis[adj[start][0]]) {
            if(adj[start].size() == 1) break;
            adj[start][0] = adj[start][1];
            if(vis[adj[start][0]]) {
                break;
            }
        }
        vis[start] = 1;
        lst = rev(lst);
        ans[{start, adj[start][0]}] = lst;
        ans[{adj[start][0], start}] = lst;
        start = adj[start][0];
    }
    for(int i = 0; i < n-1; i++) {
        cout << ans[edges[i]] << " ";
