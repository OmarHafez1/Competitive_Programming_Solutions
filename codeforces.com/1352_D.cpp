...
**
**   author:  Omar_Hafez
**   created: 12/10/2021  04:08:28 PM
**
*/
...
void solve(bool alic, int ta, int tb) {
  if(alic) {
    if(ta <= tb && !dq.empty()) moves++;
    while(!dq.empty() && ta <= tb) {
      ta += dq.front();
      a += dq.front();
      dq.popf;
    }
    ...
    if(tb <= ta && !dq.empty()) moves++;
    while(!dq.empty() && tb <= ta) {
      tb += dq.back();
      b += dq.back();
      dq.popb;
    }
