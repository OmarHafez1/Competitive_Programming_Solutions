...
**
**   author:  Omar_Hafez
**   created: 28 February 2022 (Monday)  3:50:38 PM
**
*/
...
int main() { 
  ...
  vpi ans;
  for(int i = 0; i < n; i++) {
    if(ids[a[i]].back() == -1) continue;
    if(ids[a[i]].size() == 1) {
      ans.push_back({a[i], 0});
      continue;
    }
    int dif = ids[a[i]][1]-ids[a[i]][0];
    for(int x = 1; x < ids[a[i]].size(); x++) {
      if(ids[a[i]][x]-ids[a[i]][x-1] != dif) {
        dif = -1;
        break;
      }
    }
    if(dif != -1) {
      ans.push_back({a[i], dif});
    }
    ids[a[i]].pushb(-1);
  }
  sor(ans);
