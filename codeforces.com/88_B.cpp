...
**
**   author:  Omar_Hafez
**   created: 13 February 2022 (Sunday)  1:09:48 AM
**
*/
...
int main() { 
  ...
  cin >> n >> m >> x;
  x*=x;
  vec<vec<char>> a(n, vector<char> (m));
  vb ok(300);
  vec<vpi> chs(300);
  for(int i = 0; i < n; i++) {
    for(int j = 0; j < m; j++) {
      cin >> a[i][j];
      chs[a[i][j]].pb(make_pair(i, j));
      ok[a[i][j]] = 1;
    }
  }
  int dis = 0;
  ...
  for(int i = (int)'a'; i <= 'z'; i++) {
    ...
    fe(f1, chs[i]) {
      bool stop = 0;
      fe(f2, chs['S']) {
        dis = min(dis, (int)pow(f1.first-f2.first, 2) + (int)pow(f1.second - f2.second, 2));
        if(dis <= x) {
          stop = 1;
          break;
        }
