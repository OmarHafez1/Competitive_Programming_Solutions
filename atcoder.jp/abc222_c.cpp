...
**
**   author:  Omar_Hafez
**   created: 09/10/2021  02:23:19 PM
**
*/
...
int main() { 
  ...
  f0r(j, m) {
    for(int i = 0; i < n; i++) {
      int tmp = check(a[2*n-ranks[2*i].sec][j], a[2*n-ranks[2*i+1].sec][j]);
      if(tmp == 0) {
        ranks[2*i].fir++;
      } else if(tmp == 1) ranks[2*i+1].fir++;
    }
    rsor(ranks);
  }
  fe(x, ranks) cout << 2*n-x.sec+1 << endl;
}
