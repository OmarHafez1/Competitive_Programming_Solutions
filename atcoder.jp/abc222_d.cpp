...
**
**   author:  Omar_Hafez
**   created: 09/10/2021  04:52:14 PM
**
*/
...
ll dp(int ind, int start) {
  if(ind == n) return 1;
  if(start > b[ind]) return 0;
  if(memory[ind][start] != -1) return memory[ind][start];
  start = max(start, a[ind]);
  return memory[ind][start] = (dp(ind+1, start) + dp(ind, start+1))%mod;
}
