...
**
**   author:  Omar_Hafez
**   created: 08/08/2021  02:18:07 PM
**
*/
...
int main() { 
  ...
  sor(a);
  vpi ans(n);
  int last = a[0].fir.fir, tmp;
  a[0].fir.fir = 1;
  foor(i, 1, n) {
  	tmp = a[i].fir.fir;
  	if(a[i].fir.fir == last) {
  		a[i].fir.fir = a[i-1].fir.fir;
  	} else {
  		a[i].fir.fir = a[i-1].fir.fir+1;
  	}
  	last = tmp;
  }
  sort(a.begin(), a.end(), [](auto &left, auto &right) {
    return left.fir.second < right.fir.second;
	});
  ...
  foor(i, 1, n) {
  	tmp = a[i].fir.sec;
  	if(a[i].fir.sec == last) {
  		a[i].fir.sec = a[i-1].fir.sec;
  	} else {
  		a[i].fir.sec = a[i-1].fir.sec+1;
  	}
  	last = tmp;
  }
  sort(a.begin(), a.end(), [](auto &left, auto &right) {
