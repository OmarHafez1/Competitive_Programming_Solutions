//============================================================================
// Author      : Omar_Hafez
// Created     : 17 July 2022 (Sunday)  3:35:48 PM
//============================================================================
...
ld solve(int ind, int cnt) {
	if(ind == n && cnt < mx) return 0;
	if(ind >= n) return 1.0;
	if(dp[ind][cnt] != -1) return dp[ind][cnt]; 
	return dp[ind][cnt] = solve(ind+1, cnt+1)*a[ind]+solve(ind+1, cnt)*(1-a[ind]);
}
...
signed main() { 
    ...
    cin >> n;
    dp = vector<vector<ld>> (n+1, vector<ld>(n+1, -1));
    for(int i = 0; i < n; i++) {
    	cin >> a[i];
    }
    mx = ceil(n*1.0/2);
