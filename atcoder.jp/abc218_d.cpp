...
**
**   author:  Omar_Hafez
**   created: 15/09/2021  07:43:20 PM
**
*/
...
int main() { 
  ...
  for(int i = 0; i < n; i++) {
    for(int j = i+1; j < n; j++) {
        if(xy[i].fir == xy[j].fir || xy[i].sec == xy[j].sec) continue;
        if(binary_search(all(xy), mp(xy[i].fir, xy[j].sec)) && binary_search(all(xy), mp(xy[j].fir, xy[i].sec))) ans++;
    }
  }
  cout << ans/2;
}
