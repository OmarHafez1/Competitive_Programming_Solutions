...
**
**   author:  Omar_Hafez
**   created: 21/10/2021  12:14:48 AM
**
*/
...
int solve(int ind, int sm) {
  if(ind == n) return max(sum-sm, sm);
  if(sm <= 0) return INT_MAX;
  if(dp[ind][sm] != -1) return dp[ind][sm]; 
  return dp[ind][sm] = min(solve(ind+1, sm), solve(ind+1, sm-a[ind]));
}
