...
**
**   author:  Omar_Hafez
**   created: 11/05/2021  12:46:41 PM 
**
*/
...
vi split(string s, string del = " ") {
  vi tmp;
    ...
    int end = s.find(del);
    while (end != -1) {
      tmp.pb(stoi(s.substr(start, end - start)));
        start = end + del.size();
...
int main() { 
  ...
  vec<vi> up(n+2, vi(n+2, INT_MAX)), down(n+2, vi(n+2, INT_MAX)), dp(n+2, vi(n+2));
  for(int i = 1; i <= n; i++) {
    for(int j = 1; j <= n; j++) {
      down[i][j] = v[i-1][j-1];
      up[i][j] = down[i][j];
  ...
  for(int i = 3; i < n; i++) {
    for(int j = 1; j <= n; j++) {
      down[j][i] += min(down[j][i-1], down[j-1][i]);
    }
    for(int j = n; j > 0; j--) {
      up[j][i] += min(up[j][i-1], up[j+1][i]);
    }
    for(int j = 1; j <= n; j++) {
      dp[j][i] = min(down[j][i], up[j][i]);
      down[j][i] = dp[j][i];
      up[j][i] = dp[j][i];   
    }
  }
