...
**
**   author:  Omar_Hafez
**   created: 24 March 2022 (Thursday)  2:26:52 AM
**
*/
...
int main() { 
  ...
  int ans = 0;
  int N = 40;
  vec<vi> adj(N+1, vi(N+1));
  vec<vi> dd(N+1);
  for(int i = 0; i < N; i++) {
    cin >> tmp;
    vs sss = split(tmp, ",");
    for(int j = 0; j < N; j++) {
      adj[i+1][j+1] = (sss[j] == "-"? 0 : stoi(sss[j]));
      adj[j+1][i+1] = adj[i+1][j+1];
      ans += adj[i+1][j+1];
      if(sss[j] != "-") {
        dd[i+1].push_back(j+1);
        dd[j+1].push_back(i+1);
      }
    }
  }
  ans /= 2;
  ...
  while(!q.empty()) {
    ...
    vis[tmp] = 1;
    fe(x, dd[tmp]) {
      if(vis[x] || cost[x] <= adj[x][tmp]) continue;
      q.push({adj[x][tmp], x});
      cost[x] = adj[x][tmp];
      parent[x] = tmp;
    }
