...
**
**   author:  Omar_Hafez
**   created: 17 February 2022 (Thursday)  12:29:09 PM
**
*/
...
bool good(int x, int y, int n) {
  for(int i = 0; i < 9; i++) {
    if(grid[y][i] == n || grid[i][x] == n) return 0;
  }
  x /= 3;
  y /= 3;
  x *= 3;
...
bool ok() {
  for(int i = 0; i < 9; i++) {
    for(int j = 0; j < 9; j++) {
      if(grid[i][j] == 0 || !good(j, i, grid[i][j])) return 0;
    }
  }
  return 1;
}
...
bool solve() {
  int x, y;
  if(!next(x, y)) return 1;  
  for(int i = 1; i <= 9; i++) {
    if(!good(x, y, i)) continue;
    grid[y][x] = i;
    if(solve()) return 1;
    grid[y][x] = 0;
  }
