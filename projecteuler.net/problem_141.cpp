...
namespace NT{
    ...
    template<typename T>
    typename enable_if<is_integral<T>::value, vector<pair<T, int>>>::type factorize_count(T x){
        vector<pair<T, int>> ret;
        vector<T> tmp = NT::factorize(x);
        int lst = 0, cnt = 0;
        for(auto &w : tmp) {
            if(lst == 0) lst = w;
            if(w == lst) {
...
void go(int n, int r, int ind, vector<pair<int, signed>> &ps) {
    if(r > n) return;
    if(ind == ps.size()) {
        int m = n*n/r;
        vector<int> tmp = {n, r, m};
        sort(tmp.begin(), tmp.end());
        if(tmp[0] == tmp[1] || tmp[1] == tmp[2]) return;
        if(isP2(tmp[1]*tmp[2]+tmp[0]))
            st.insert(tmp[1]*tmp[2]+tmp[0]);
        if(isP2(tmp[2]*tmp[0]+tmp[1]))
            st.insert(tmp[2]*tmp[0]+tmp[1]);
        return;
    }
    for(int i = 0; i <= ps[ind].second; i++) {
        go(n, r*pow(ps[ind].first, i), ind+1, ps);
    }
}
