//============================================================================
// Author      : Omar_Hafez
// Created     : 17 March 2023 (Friday)  7:31:29 PM
//============================================================================
...
void go(int u, int key, int sz, int start) {
    if(u == start) {
        len[start] = max(len[start], sz);
        return;
    }
    if(keys[u] == key) return;
    keys[u] = key;
    if(nxt[u] <= N) go(nxt[u], key, sz+1, start);
}

int find_min(int u, int start) {
    if(u == start) {
        return start;
    }
    return min(u, find_min(nxt[u], start));
}
