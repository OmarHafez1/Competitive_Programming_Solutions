//============================================================================
// Author      : Omar_Hafez
// Created     : 12 April 2023 (Wednesday)  11:45:29 AM
//============================================================================
...
int solve(int ind, int pp, int p) {
    if(ind == 20) return 1;
    int key = ind*1000+pp*10+p;
    if(cache[key] != -1) return cache[key];
    int res = 0;
    for(int i = (ind==0); pp+p+i < 10; i++) {
        res += solve(ind+1, p, i);
    }
...
signed main() { 
    ...
    for(int i = 1; i < 20; i++) {
        for(int pp = 0; pp < 10; pp++) {
            for(int p = 0; p < 10; p++) {
                for(int c = 0; p+pp+c < 10; c++) {
                    dp[i][p][c] += dp[i-1][pp][p];
                }
            }
        }
    }
