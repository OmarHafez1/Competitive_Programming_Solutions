...
vector<bool> isPrime;
void go(int a, int b, int ind, vector<pair<int, signed>> &ps) {
    if(ind == ps.size()) {
        int c = b*b/a;
        if(a < b && b < c && c <= N && isPrime[a-1] && isPrime[c-1]) {
            ans += a+b+c-3;
        }
        return; 
    }
    for(int i = 0; i <= ps[ind].second; i++) {
        go(a*pow(ps[ind].first, i), b, ind+1, ps);
    }
}

signed main() { 
    cin >> N;
    isPrime = primes_sieve(N+10);
    for(int i = 3; i <= N; i+=2) {
        if(!isPrime[i]) continue;
        auto tmp = NT::factorize_count(i+1);
        go(1, i+1, 0, tmp);
    }
    cout << ans;
} 
