//============================================================================
// Author      : Omar_Hafez
// Created     : 06 May 2023 (Saturday)  5:36:54 PM
//============================================================================
...
signed main() { 
    ...
    if(ods.back() != n) ods.push_back(n);
    int start = 0; 
    int ans = 0;
    for(int i = 1; i < ods.size()-1; i++) {
        ans = max(ans, ods[i]-ods[i-1]+ods[i+1]-ods[i]-1);
    }
    ans = max(ans, ods[2]-1);
    if(tmp%2 == 0)
        ans = max(ans, n-ods[ods.size()-3]);
    cout << ans;
