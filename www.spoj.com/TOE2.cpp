...
**
**   author:  Omar_Hafez
**   created: 19 February 2022 (Saturday)  11:12:58 AM
**
*/
...
bool winnig(string str) {
  for(int i = 0; i < 3; i++) {
    if(str[i] == str[i+3] && str[i+3] == str[i+6] && str[i] != '.') return 1;
    if(str[i*3] == str[i*3+1] && str[i*3+1] == str[i*3+2] && str[i*3] != '.') return 1;
  }
  if(str[0] == str[4] && str[4] == str[8] && str[8] != '.') return 1;
  if(str[2] == str[4] && str[4] == str[6] && str[2] != '.') return 1;
  return 0;
}
...
void solve(string &grid) {
  ...
  while(!q.empty()) {
    ...
    if(winnig(tmp)) continue;
    for(int i = 0; i < 9; i++) {
      if(grid[i] == '.') continue;
      if(tmp[i] != '.') continue;
      if(ch != grid[i]) continue;
      tmp[i] = ch;
      q.push({tmp, invert(ch)});
      tmp[i] = '.';
    }
  }
  cout << "invalid";
