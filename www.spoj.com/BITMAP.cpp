...
**
**   author:  Omar_Hafez
**   created: 23 June 2022 (Thursday)  11:44:16 PM
**
*/
...
void calculate() {
    ...
    vector<vector<int>> board(n, vi(m, INT_MAX));
    for(int i = 0; i < w.size(); i++) {
    	int i1 = w[i].first, j1 = w[i].second;
    	board[i1][j1] = 0;
    	for(int j = 0; j < b.size(); j++) {
    		int i2 = b[j].first, j2 = b[j].second;
    		board[i2][j2] = min(board[i2][j2], abs(i1-i2)+abs(j1-j2));
    	}
    }
    for(int i = 0; i < n; i++) {
    	for(int j = 0; j < m; j++) {
