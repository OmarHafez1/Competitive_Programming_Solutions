...
**
**   author:  Omar_Hafez
**   created: 19 February 2022 (Saturday)  8:44:12 AM
**
*/
...
bool is_win(string str) {
  for(int i = 0; i < 3; i++) {
    if(str[i] == str[i+3] && str[i+3] == str[i+6] && str[i+3] != '.') return 1;
    if(str[i*3] == str[i*3+1] && str[i*3+1] == str[i*3+2] && str[i*3+2] != '.') return 1;
  }
  if(str[0] == str[4] && str[4] == str[8] && str[8] != '.') return 1;
  if(str[2] == str[4] && str[4] == str[6] && str[4] != '.') return 1;
  return 0;
}
