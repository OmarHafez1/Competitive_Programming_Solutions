//============================================================================
// Author      : Omar_Hafez
// Created     : 13 May 2023 (Saturday)  11:33:32 PM
//============================================================================
...
void go(int a[], vector<int> &lis) {
    vector<int> dp(n);
    int k = 0;
    for(int i = 0; i < n; i++) {
        int pos = lower_bound(dp.begin(), dp.begin()+k, a[i])-dp.begin();
        dp[pos] = a[i];
        lis[i] = pos+1;
        k += (pos == k);
    }
...
signed main() { 
    ...
    while(cin >> n) {
        ...
        int ans = 1;
        for(int i = 0; i < n; i++) {
            ans = max(ans, 2*min(lis[i], lds[n-1-i])-1);
        }
        cout << ans << endl;
    }
