//============================================================================
// Author      : Omar_Hafez
// Created     : 14 July 2023 (Friday)  5:46:54 PM
//============================================================================
...
void dfs2(bool &ans2, int u, vector<vector<int>> &adj, vector<int> &cat, vector<int> &mice) {
    if(cat[u]) return;
    if(mice[u] == 2) {
        if(u == m)
            ans2 = 1;
...
void calculate() {
    ...
    cin.ignore();
    int u, v;
    vector<vector<int>> adjc(n+1), adjm(n+1);
    vector<int> cat(n+1), mice(n+1);
    bool fir = 0;
    while(getline(cin, s) && !s.empty()) {
        ...
        if(!fir) adjc[u].push_back(v);
        else adjm[u].push_back(v);
    }
    dfs(c, adjc, cat);
    dfs(m, adjm, mice);
    bool ans1 = 0, ans2 = 0;
