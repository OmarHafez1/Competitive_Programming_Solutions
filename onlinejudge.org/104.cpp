//============================================================================
// Author      : Omar_Hafez
// Created     : 13 July 2023 (Thursday)  7:44:15 PM
//============================================================================
...
signed main() { 
    ...
    while(cin >> n) {
        ...
        vector<vector<vector<int>>> parent(n+1, vector<vector<int>>(n+1, vector<int>(n+1, 0)));
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) {
                parent[i][j][1] = i;
                if(i == j) {
        ...
        bool done = 0;
        for(int step = 2; step <= n && !done; step++) {
            for(int k = 0; k < n; k++) {
                for(int i = 0; i < n; i++) {
                    for(int j = 0; j < n; j++) {
                        if(dp[i][j][step] < dp[i][k][1]*dp[k][j][step-1]) {
                            dp[i][j][step] = dp[i][k][1]*dp[k][j][step-1];
                            parent[i][j][step] = parent[k][j][step-1];
                        }
                    }
                }
            }
