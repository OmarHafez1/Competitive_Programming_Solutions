//============================================================================
// Author      : Omar_Hafez
// Created     : 10 July 2023 (Monday)  12:29:42 AM
//============================================================================
...
signed main() { 
    ...
    while(cin >> n >> m) {
        ...
        pdqu<tuple<int, bool, int>> q;
        q.push({0, 0, 0});
        vector<vector<int>> dp(2, vector<int>(n+10, INF));
        dp[0][0] = 0;
        while(!q.empty()) {
            bool state = get<1>(q.top());
            int cost = get<0>(q.top());
            u = get<2>(q.top());
            q.pop();
            state = !state;
            fe(x, adj[u]) {
                if(cost+x.second < dp[state][x.first]) {
                    dp[state][x.first] = cost+x.second;
                    q.push({dp[state][x.first], state, x.first});
                }
            }
        }
        if(dp[0][n-1] == INF) {
