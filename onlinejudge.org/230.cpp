//============================================================================
// Author      : Omar_Hafez
// Created     : 04 September 2022 (Sunday)  9:09:06 PM
//============================================================================
...
bool comb(string a, string b, string c, string d) {
    if(a < c) return 1;
    if(a == c) return b <= d;
    return 0;
}
...
signed main() { 
    ...
    while(cin >> s) {
        ...
        if(s == "SHELVE") {
            ...
            int ind = 0;
            if(lst.empty()) {
                lst.push_back(ret[0].second);
                cout << "Put \"" << ret[0].second << "\" first" << endl; 
                ind++;
            }
            for(int i = ind; i < ret.size(); i++) {
                auto it = lst.begin();
                while(it != lst.end() && comb(auth[*it], *it, auth[ret[i].second], ret[i].second)) it++;
                if(it == lst.begin()) {
                    cout << "Put \"" << ret[i].second << "\" first" << endl; 
                } else {
                    string tmp = *prev(it);
                    lst.insert(it, ret[i].second);
                    it = prev(it);
                    cout << "Put \"" << ret[i].second << "\" after \"" << tmp << "\"" << endl;
                }
            }
            cout << "END" << endl;
            ret.clear();
