//============================================================================
// Author      : Omar_Hafez
// Created     : 11 July 2023 (Tuesday)  8:59:10 PM
//============================================================================
...
struct node {
    int cost, tank, u;
    node (int cost, int tank, int u) : cost(cost), tank(tank), u(u) {}
    ...
    while(cin >> n >> m) {
        ...
        while(quer--) {
            ...
            while(!q.empty()) {
                auto t = q.top(); q.pop();
                if(t.tank < c) {
                    if(dp[t.u][t.tank+1] > t.cost+prices[t.u]) {
                        dp[t.u][t.tank+1] = t.cost+prices[t.u];
                        q.push(node(t.cost+prices[t.u], t.tank+1, t.u));
                    }
                }
                for(auto &x : adj[t.u]) {
                    if(t.tank >= x.second) {
                        if(t.cost < dp[x.first][t.tank-x.second]) {
                            dp[x.first][t.tank-x.second] = t.cost;
                            q.push(node(t.cost, t.tank-x.second, x.first));
                        }
                    } 
                }
            }
            if(dp[e][0] == INF) cout << "impossible" << endl;
            else cout << dp[e][0] << endl;
        }
    }
