//============================================================================
// Author      : Omar_Hafez
// Created     : 11 July 2023 (Tuesday)  1:33:26 PM
//============================================================================
...
signed main() { 
    ...
    while(cin >> n >> k) {
        ...
        for(int i = 0; i < n; i++) {
            ...
            for(int j = 0; j < a.size(); j++) {
                for(int k = j+1; k < a.size(); k++) {
                    adj[a[j]].push_back({a[k], i});
                    adj[a[k]].push_back({a[j], i});
                }
            }
        }
        ...
        while(!q.empty()) {
            ...
            int elevator = get<1>(q.top());
            int floor = get<2>(q.top());
            q.pop();
            for(auto &x : adj[floor]) {
                int tmp = cost + (elevator != x.second)*60 + abs(x.first-floor)*t[x.second];
                if(tmp < dp[x.first][x.second]) {
                    dp[x.first][x.second] = tmp;
                    q.push({tmp, x.second, x.first});
                }
            }
        }
        int ans = INF;
