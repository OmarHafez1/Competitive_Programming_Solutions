//============================================================================
// Author      : Omar_Hafez
// Created     : 11 July 2023 (Tuesday)  3:42:44 PM
//============================================================================
...
bool ok(ld x1, ld y1, ld x2, ld y2) {
    x1 -= x2; y1 -= y2;
    x1 *= x1; y1 *= y1;
    x1 += y1;
    return (x1 <= 2.25);
...
signed main() { 
    ...
    while(1) {
        ...
        pdqu<pair<ld, int>> q;
        for(int i = 0; i < n; i++) {
            if(ok(a[i].first, a[i].second, 0, 0)) {
                dp[i] = dis(a[i].first, a[i].second, 0, 0);
                q.push({dp[i], i});
            }
        }
        while(!q.empty()) {
            ...
            q.pop();
            for(auto &x : adj[u]) {
                if(ok(a[x].first, a[x].second, a[u].first, a[u].second)) {
                    ld tmp = cost + dis(a[x].first, a[x].second, a[u].first, a[u].second);
                    if(tmp < dp[x]) {
                        dp[x] = tmp;
                        q.push({tmp, x}); 
                    }                   
        ...
        cout << ((dp[n-1] <= d)? "I am lucky!" : "Boom!") << endl;
    }
}
