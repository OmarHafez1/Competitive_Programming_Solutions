//============================================================================
// Author      : Omar_Hafez
// Created     : 10 July 2023 (Monday)  7:27:24 PM
//============================================================================
...
void calculate() {
    int w[4];
    cin >> w[0] >> w[1] >> w[2] >> w[3];
    vector<vector<int>> dp(w[0]+1, vector<int>(w[1]+1, INF));
    pdqu<tuple<int, int, int, int>> q;
    q.push({0, 0, 0, w[2]});
    dp[0][0] = 0;
    while(!q.empty()) {
        ...
        for(int i = 0; i < 3; i++) {
            for(int j = 0; j < 3; j++) {
                int cst[3] = {a, b, c};
                if(i == j || cst[j] == 0 || cst[i] == w[i]) continue;
                int want = min(w[i]-cst[i], cst[j]);
                cst[i] += want;
                cst[j] -= want;
                if(dp[a][b]+want < dp[cst[0]][cst[1]]) {
                    dp[cst[0]][cst[1]] = dp[a][b]+want;
                    q.push({dp[a][b]+want, cst[0], cst[1], cst[2]});
                }
            }
        }
    }
    ...
    for(int d = w[3]; ~d; d--) {
        ...
        for(int i = 0; i <= w[0]; i++) {
            if(w[2]-i-d >= 0 && w[2]-i-d <= w[1])
                ans = min(ans, dp[i][w[2]-d-i]);
        }
        if(ans != INF) {
            cout << ans << " " << d;
            return;
