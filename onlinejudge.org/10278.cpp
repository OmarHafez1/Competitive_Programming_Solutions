//============================================================================
// Author      : Omar_Hafez
// Created     : 09 July 2023 (Sunday)  10:57:03 PM
//============================================================================
...
void calculate() {
    ...
    for(int i = 0; i < f; i++) {
        ...
        while(!q.empty()) {
            ...
            q.pop();
            for(auto &x : adj[u]) {
                if(dp[u]+x.second < dp[x.first]) {
                    dp[x.first] = dp[u]+x.second;
    ...
    for(int i = 1; i <= n; i++) {
        ...
        while(!q.empty()) {
            ...
            q.pop();
            for(auto &x : adj[u]) {
                if(dp2[u]+x.second < dp2[x.first]) {
                    dp2[x.first] = dp2[u]+x.second;
                    q.push({dp2[x.first], x.first});
                }
            }
        }
        int tx = 0;
        for(int j = 1; j <= n; j++) {
            if(dp2[j] > tx) tx = dp2[j];
        }
        if(tx < mx) {
            mx = tx;
            ans = i;
