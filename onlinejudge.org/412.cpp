...
**
**   author:  Omar_Hafez
**   created: 10 February 2022 (Thursday)  7:09:10 PM
**
*/
...
ull comb(int n, int m, vec<vull> &dp) {
  if(m > n) return 0;
  if(n == 0 || m == 0) return 1;
  if(dp[n][m] != -1) return dp[n][m];
  return dp[n][m] = comb(n-1, m, dp) + comb(n-1, m-1, dp);
} 
...
int main() { 
  ...
  while(cin >> n) {
    ...
    vec<vull> dp(n+1, vull(n+1, -1));
    cout << sqrt(6.0/(cnt*1.0/comb(n, 2, dp))) << endl;
  }
