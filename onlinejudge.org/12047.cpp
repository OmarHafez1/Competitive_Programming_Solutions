//============================================================================
// Author      : Omar_Hafez
// Created     : 12 July 2023 (Wednesday)  1:50:58 PM
//============================================================================
...
void dijk(vector<vector<pair<int, int>>> &adj, vector<int> &dp, int s, int e) {
    pdqu<pair<int, int>> q;
    q.push({0, s});
    dp[s] = 0;
    while(!q.empty()) {
...
void calculate() {
    ...
    for(int i = 1; i <= n; i++) {
        for(auto &x : adj[i]) {
            if(x.second + dp1[i] + dp2[x.first] <= p) {
