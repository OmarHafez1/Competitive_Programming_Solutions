//============================================================================
// Author      : Omar_Hafez
// Created     : 29 May 2023 (Monday)  7:49:52 AM
//============================================================================
...
void solve(int u, int parent, int time, vector<vector<int>> &adj, vector<int> &dis, vector<int> &mn, vector<pair<int, int>> &bridges) {
    dis[u] = mn[u] = ++time;
    fe(v, adj[u]) {
        ...
        if(dis[v]) {
            ...
            solve(v, u, time, adj, dis, mn, bridges);
            if(dis[u] < mn[v]) {
...
signed main() { 
    ...
    while(cin >> n) {
        ...
        for(int i = 0; i < n; i++) {
            if(!dis[i]) {
                solve(i, -1, 0, adj, dis, mn, bridges);
            }
        }
        sor(bridges);
