...
**
**   author:  Omar_Hafez
**   created: 18/12/2021  11:33:13 PM
**
*/
...
bool leap_year(int y) {
  return (y%4 == 0 && (y%100==0? y%400==0 : 1));
} 

bool ok(Star d1, Star d2) {
  if(d1.year != d2.year) {
    if(d2.year - d1.year == 1) {
      if(d2.month == 1 && d1.month == 12 && d2.day == 1 && d1.day == 31) {
        return 1;
      }
    }
    return 0;
  ...
  if(d1.month != d2.month) {
    if(d2.month-d1.month == 1) {
      int tmp = days[d1.month-1];
      if(d1.month == 2 && leap_year(d1.year)) tmp++;
      if(d1.day == tmp && d2.day == 1) return 1;
    }
    return 0;
  }
  if(d2.day - d1.day == 1) return 1;
  return 0;
}
...
int main() { 
  ...
  while(cin >> n) {
    if(n == 0) return 0;
    vector<Star> data(n, Star());
    unsigned int cnt = 0, kwh = 0; 
    cin >> data[0].day >> data[0].month >> data[0].year >> data[0].kwh;
    foor(i, 1, n) {
      cin >> data[i].day >> data[i].month >> data[i].year >> data[i].kwh;
