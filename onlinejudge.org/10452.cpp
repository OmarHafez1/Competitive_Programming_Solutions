...
**
**   author:  Omar_Hafez
**   created: 25/12/2021  10:08:51 PM
**
*/
...
void go(vs &pth, int i, int j, int ind, string ans) {
  if(j >= m || i < 0 || j < 0 || ind >= word.len) return;
  if(pth[i][j] == '#') {
    cout << ans.substr(0, ans.len-1);
    return;
  }
  if(pth[i][j] == word[ind]) {
    pth[i][j] = '0';
    go(pth, i, j-1, ind+1, ans + "left ");
    go(pth, i, j+1, ind+1, ans + "right ");
    go(pth, i-1, j, ind+1, ans + "forth ");
  }
}
