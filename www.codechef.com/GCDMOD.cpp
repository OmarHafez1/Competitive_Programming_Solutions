...
**
**   author:  Omar_Hafez
**   created: 29 June 2022 (Wednesday)  1:15:11 PM
**
*/
...
ull exponential_mod(ull n, ull p, ull mod) {
  if(p == 0) return 1;
  if(p&1) return ((__int128)exponential_mod(n, p-1, mod)*(__int128)n)%mod;
  ull tmp = exponential_mod(n, p/2, mod)%mod;
  return ((__int128)tmp*(__int128)tmp)%mod;
}
...
void calculate() {
    ...
    if(b > a) swap(a, b);
    ull m = (a==b? MOD : a-b);
    cout << (gcd((exponential_mod(a, n, m)+exponential_mod(b, n, m))%m, a-b)%MOD);
}
